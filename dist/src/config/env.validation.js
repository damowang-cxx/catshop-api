...
function ensureProductionValue(name, value, disallowed) {
...
function validateEnv(config) {
    ...
    const validated = {
        ...
        JWT_REFRESH_SECRET: String(config.JWT_REFRESH_SECRET ?? DEFAULT_JWT_REFRESH_SECRET),
        ...
        ADMIN_PASSWORD_HASH: String(config.ADMIN_PASSWORD_HASH ?? password_util_1.DEV_ADMIN_PASSWORD_HASH),
        CUSTOMER_EMAIL: String(config.CUSTOMER_EMAIL ?? DEFAULT_CUSTOMER_EMAIL),
        CUSTOMER_PASSWORD_HASH: String(config.CUSTOMER_PASSWORD_HASH ?? password_util_1.DEV_CUSTOMER_PASSWORD_HASH),
    ...
    if (isProduction) {
        ...
        ensureProductionValue('ADMIN_PASSWORD_HASH', validated.ADMIN_PASSWORD_HASH, [
            password_util_1.DEV_ADMIN_PASSWORD_HASH,
        ]);
        ensureProductionValue('CUSTOMER_PASSWORD_HASH', validated.CUSTOMER_PASSWORD_HASH, [password_util_1.DEV_CUSTOMER_PASSWORD_HASH]);
        if (!(0, password_util_1.isArgon2Hash)(validated.ADMIN_PASSWORD_HASH)) {
            throw new Error('ADMIN_PASSWORD_HASH must be a valid Argon2id hash.');
        }
        if (!(0, password_util_1.isArgon2Hash)(validated.CUSTOMER_PASSWORD_HASH)) {
        ...
        if (String(config.ADMIN_PASSWORD ?? '').trim()) {
            throw new Error('ADMIN_PASSWORD is deprecated and must not be used in production.');
        }
    }
    if (validated.GOOGLE_AUTH_ENABLED) {
        ...
        validated.GOOGLE_REDIRECT_URI = ensureUrl('GOOGLE_REDIRECT_URI', validated.GOOGLE_REDIRECT_URI, validated.GOOGLE_REDIRECT_URI);
    }
    return validated;
}
